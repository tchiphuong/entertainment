...
// Danh sách links điều hướng (chỉ riêng phân hệ VOD)
const NAV_LINKS = [
    {
        to: "/vod",
...
const MOBILE_EXTRA_LINKS = [
    {
        ...
        labelKey: "vods.favorites",
        fallback: "Phim yêu thích",
    },
    {
        to: "/vod/category/history",
        icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z",
        labelKey: "vods.history",
        fallback: "Lịch sử xem",
    },
];
...
function LanguageDropdown({ i18n, showLangDropdown, setShowLangDropdown, changeLanguage, langRef }) {
    return (
        <div className="relative" ref={langRef}>
            ...
            >
                <span className="h-3 w-4 overflow-hidden rounded-sm ring-1 ring-white/10 md:h-3.5 md:w-5">
                    <img
                ...
                <div className="absolute right-0 mt-2 w-56 origin-top-right overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950/95 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in-95 duration-150 z-50">
                    ...
                    <div className="border-t border-zinc-800 p-1.5">
                        <button
                            ...
                            className="flex w-full items-center gap-3 rounded-full px-4 py-2.5 text-left text-xs font-bold text-red-500 transition-all hover:bg-red-500/10 cursor-pointer"
                        >
...
// Drawer điều hướng trên thiết bị di động
function MobileDrawer({ isMenuOpen, setIsMenuOpen, currentUser, t, isActiveLink, changeLanguage, i18n, getUserInitial }) {
    return (
        <>
            <button
                type="button"
                ...
                className={`fixed inset-0 z-60 bg-black/60 backdrop-blur-sm transition-opacity duration-300 lg:hidden ${
                    isMenuOpen ? "opacity-100 cursor-pointer" : "pointer-events-none opacity-0"
                }`}
            ...
            >
                ...
                <div className="flex flex-col gap-1 px-3">
                    {[...NAV_LINKS, ...(currentUser ? MOBILE_EXTRA_LINKS : [])].map((link) => (
                        <Link
                            key={link.to}
                            to={link.to}
                            onClick={() => setIsMenuOpen(false)}
