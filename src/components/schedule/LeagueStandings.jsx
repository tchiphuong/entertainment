...
const getZoneIndicatorColor = (rank, totalTeams, description = "") => {
    ...
    if (descLower.includes("champions league") || rank <= 4) return "bg-emerald-500";
    if (descLower.includes("europa league") || rank === 5) return "bg-sky-500";
    if (descLower.includes("conference") || descLower.includes("qualification") || rank === 6) return "bg-amber-500";
    if (descLower.includes("relegation") || (totalTeams > 6 && rank > totalTeams - 3)) return "bg-red-500";
    return "bg-transparent";
};

// Xác định biểu tượng biến động thứ hạng / phong độ cạnh số hạng
const MovementIndicator = ({ formStr, t }) => {
    if (!formStr) {
        return (
...
const StandingsTableRow = ({ row, totalTeams, t }) => {
    const indicatorColor = getZoneIndicatorColor(row.rank, totalTeams, row.description);
        ...
        >
            <td className="relative px-3 py-3.5 text-center">
                ...
                <div className="flex items-center justify-center gap-1.5">
                    <span className={`inline-flex h-6 w-6 items-center justify-center rounded-md text-xs font-black ${rankBadgeClass}`}>
                        {row.rank}
                    </span>
                    <MovementIndicator formStr={row.form} t={t} />
                </div>
            </td>
            ...
            <td className="px-4 py-3.5">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white tracking-wide">{row.teamName}</span>
                </div>
            </td>

            <td className="px-3 py-3.5 text-center font-medium text-zinc-300">{row.played}</td>
            <td className="px-3 py-3.5 text-center font-medium text-emerald-400">{row.win}</td>
            <td className="px-3 py-3.5 text-center font-medium text-amber-400">{row.draw}</td>
            <td className="px-3 py-3.5 text-center font-medium text-red-400">{row.loss}</td>
            <td className="px-3 py-3.5 text-center font-medium text-zinc-400 hidden md:table-cell">{row.goalsFor}</td>
            <td className="px-3 py-3.5 text-center font-medium text-zinc-400 hidden md:table-cell">{row.goalsAgainst}</td>
...
const StandingsTable = ({ standings, loadingStandings, t }) => {
    if (loadingStandings) {
        return (
...
const LeagueSelectBox = ({ leagues, selectedLeagueId, onSelectLeague, activeLeague, loadingLeagues, t }) => {
    if (loadingLeagues) {
        return (
    ...
    return (
        <Select
            ...
            onSelectionChange={(key) => onSelectLeague(String(key))}
...
const SeasonSelectBox = ({ availableSeasons, selectedSeason, onSelectSeason, t }) => (
    <Select
        selectedKey={selectedSeason}
        onSelectionChange={(key) => onSelectSeason(String(key))}
...
const LeagueStandingsHeader = ({ activeLeague, selectedSeason, t }) => (
...
export default function LeagueStandings() {
    ...
    const availableSeasons = useMemo(() => scheduleService.generateRecentSeasons(5), []);
        ...
        <div className="space-y-6">
            ...
            <div className="overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/90 shadow-2xl backdrop-blur-md">
                ...
                <StandingsTable
                    ...
                    t={t}
                />
                {standings.length > 0 && <StandingsLegendFooter t={t} />}
            </div>
        </div>
    );
}
